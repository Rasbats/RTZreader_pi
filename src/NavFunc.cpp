...
double bearingFromCoordinates(double lat1, double long1, double lat2,
	double long2) {

	double dLon = (long2 - long1) * d2r;

	double y = sin(dLon) * cos(lat2 * d2r);
	double x = cos(lat1* d2r) * sin(lat2 *d2r) - sin(lat1 * d2r)
		* cos(lat2 * d2r) * cos(dLon);

	double brng = atan2(y, x);

	double brng2 = brng * r2d + 360;
	brng2 = fmod(brng2, 360);
	//brng = 360 - brng; // count degrees counter-clockwise - remove to make clockwise

	return brng2;
}

wxRealPoint ProjectPointToWebMercator(wxRealPoint lonlat) {  //x.y
	double RadiansPerDegree = M_PI / 180;
	double Rad = lonlat.y * RadiansPerDegree;
	double FSin = sin(Rad);
	double DegreeEqualsRadians = 0.017453292519943;
	double EarthsRadius = 6378137;

	double y = EarthsRadius / 2.0 * log((1.0 + FSin) / (1.0 - FSin));
	double x = lonlat.x * DegreeEqualsRadians * EarthsRadius;

	wxRealPoint webLonLat;
	webLonLat.x = x;
	webLonLat.y = y;

	wxString outx = wxString::Format("%f", x);
	wxString outy = wxString::Format("%f", y);
	//wxMessageBox(outy);
...
wxRealPoint WebMercatorToLonLat(wxRealPoint weblonlat) {  //x.y

	wxRealPoint retval;

	double originShift = 2 * M_PI * 6378137 / 2.0;

	retval.x = (weblonlat.x / originShift) * 180.0;
	retval.y = (weblonlat.y / originShift) * 180.0;

	retval.y = 180 / M_PI * (2 * atan(exp(retval.y * M_PI / 180.0)) - M_PI / 2.0);

	return retval;

}

double calcCentreOfPivotDirection(double firstCse, double secondCse, wxString &portstbd, double &halfangle) {
	//
	/*
	...
	*/
	double twa = 360 - ((firstCse - secondCse) - 360);
	//double twa = 180 - abs(secondCse - firstCse);
	...
	//wxMessageBox(portstbd);

	

	twa = (180 - twa)/2;  // we need half the complement of the twa for the internal angle of the triangle
	//twa = twa * M_PI / 180; // convert to radians
	// degrees needed for navfunc methods
	//second course is 045;

    wxString twind = wxString::Format("%f", twa);
	//wxMessageBox(twind);
